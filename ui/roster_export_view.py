...
def open_roster_export(parent, db, base_dir, school_year, context=""):
    RosterExportDialog(parent, db, base_dir, school_year, context)


class RosterExportDialog(ttk.Toplevel):
    def __init__(self, parent, db, base_dir, school_year, context=""):
        super().__init__(parent)
        self.db = db
        self.base_dir = base_dir
        self.school_year = school_year
        self.title("Export Roster to Excel")
        self.grab_set()
        self.lift()
        ...
        ttk.Label(self, text="Export a Student Name / Grade / Student ID list to "
                             "Excel.", font=("Segoe UI", fs(10), "bold")).pack(
